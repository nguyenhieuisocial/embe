...
const dir=resolve('data/studio-story-verification');await mkdir(dir,{recursive:true});
...
try {
  for(const voice of ['thuc-doan-south-v1','my-duyen-south-v1']){
  ...
  for(const [voice,name] of [['thuc-doan-south-v1','Thục Đoan'],['my-duyen-south-v1','Mỹ Duyên'],['ai-han-south','Ái Hân']]){
  ...
  if(existing.status()===404){
    const r=await context.request.post(origin+'/api/studio/workspace',{headers:{origin},data:{action:'save',id,revision:0,payload:{title,stage:'Nghe thử giọng AI',caption:'Mẫu tự viết. Không phải nội dung y tế, không đăng mạng xã hội.',voice:{id:'thuc-doan-south-v1',speed:1},scenes:[{heading:'Cùng mẹ, thật nhẹ nhàng',text}],sources:[{title:'VieNeu Turbo',url:'https://huggingface.co/pnnbao-ump/VieNeu-TTS-v3-Turbo'}]}}});
  ...
  await page.getByLabel('Chọn giọng').selectOption('my-duyen-south-v1');
  await page.getByRole('button',{name:'Nghe mẫu Mỹ Duyên'}).waitFor();
  await page.getByLabel('Chọn giọng').selectOption('thuc-doan-south-v1');
  ...
  if(project.payload.voice.id!=='thuc-doan-south-v1'||project.payload.voice.speed!==speed||project.payload.scenes[0].text!==text)throw new Error('voice_not_saved');
...
}catch(e){result.status='failed';result.error=/^[a-zA-Z0-9_]+$/.test(e.message)?e.message:'verification_failed';result.line=e.stack?.match(/studio-story-voice-live\.mjs:(\d+)/)?.[1];await page.screenshot({path:resolve(dir,'failure.png'),fullPage:true}).catch(()=>{});process.exitCode=1;}
