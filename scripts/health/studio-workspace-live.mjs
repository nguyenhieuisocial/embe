...
try{
  ...
  await page.goto(`${origin}/studio/kham-pha`,{waitUntil:'domcontentloaded'});await page.getByRole('button',{name:'Lưu ý tưởng',exact:true}).waitFor();const board=await(await context.request.get(`${origin}/api/studio/workspace?board=1`)).json();result.notebookCloud=Number.isInteger(board.revision)&&Array.isArray(board.items);
