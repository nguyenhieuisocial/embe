...
function Get-AgeHours([string]$Value) {
    return ($now - [DateTimeOffset]::Parse($Value)).TotalHours
}

if ($FixturePath) {
    $fixture = Get-Content -LiteralPath $FixturePath -Raw | ConvertFrom-Json
    if ($fixture.now_utc) { $now = [DateTimeOffset]::Parse($fixture.now_utc) }
    ...
    $backupCreated = [string]$fixture.backup_created_utc
    $restoreStatus = [string]$fixture.restore.status
    $restoreVerified = [string]$fixture.restore.verified_at
    $integrityStatus = [string]$fixture.integrity.status
    $integrityChecked = [string]$fixture.integrity.checked_at
    ...
    $backupCreated = if ($latestManifest) { [string](Get-Content $latestManifest.FullName -Raw | ConvertFrom-Json).created_utc } else { "" }
    ...
    $restoreVerified = if ($restore) { [string]$restore.verified_at } else { "" }
    ...
    $integrityChecked = if ($integrity) { [string]$integrity.checked_at } else { "" }
...
$portalAge = if ($portalHealthy) { Get-AgeHours ([string]$portalStatus.last_success_at) } else { [double]::PositiveInfinity }
...
$bridgeAge = if ($bridgeHealthy) { Get-AgeHours ([string]$bridgeStatus.time) } else { [double]::PositiveInfinity }
