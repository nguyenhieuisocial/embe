...
try {
  ...
  // The notebook is shared cloud data now. Keep this broad smoke check read-only;
  // mutation/conflict handling has its own bounded workspace verification.
  const board = await (await context.request.get(`${origin}/api/studio/workspace?board=1`)).json();
  if (!Number.isInteger(board.revision) || !Array.isArray(board.items)) throw new Error('discovery_cloud_notebook');
  await page.getByText(/^Lưu riêng trên EmBe, mở được từ các điện thoại/).waitFor();
  ...
  result.discovery = { googleTrendsReady: true, cacheReused: true, multilingual: true, cloudNotebookRead: true, sharedNotebookUnchanged: true };
