...
begin
  if p_days not between 7 and 30 then raise exception 'invalid context period'; end if;
  select jsonb_build_object(
    'due_date', (select due_date from portal_read_model.pregnancy_profile where singleton),
    'health', coalesce((select jsonb_agg(to_jsonb(item) order by item.day) from (
      ...
      where day >= current_date - (p_days - 1) order by day desc limit 30
    ) item), '[]'::jsonb),
    'meals', coalesce((select jsonb_agg(to_jsonb(item) order by item.eaten_at) from (
      select eaten_at, meal_type, note, confirmed_analysis -> 'foods' as foods,
    ...
    'active_care_plans', coalesce((select jsonb_agg(to_jsonb(item) order by item.name) from (
    ...
    'upcoming_appointments', coalesce((select jsonb_agg(to_jsonb(item) order by item.occurred_at) from (
