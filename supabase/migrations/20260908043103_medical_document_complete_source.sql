BEGIN;
-- Immutable independent transcription, captured from the trusted scan in the same
-- import transaction. Do not copy caller-provided PDF/OCR text into this column.
ALTER TABLE portal_read_model.medical_document_import
 ADD COLUMN source_pages jsonb NOT NULL DEFAULT '[]'::jsonb,
 ADD CONSTRAINT medical_import_source_bounded CHECK(jsonb_typeof(source_pages)='array'
  AND jsonb_array_length(source_pages)<=6 AND octet_length(source_pages::text)<=1250000);
...
BEGIN
 ...
 IF p_details ? 'nextAppointmentAt' AND p_details->>'nextAppointmentAt' IS NOT NULL THEN
  ...
  IF (next_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date < GREATEST((p_details->>'occurredOn')::date,
   CASE WHEN r.document_intake THEN (p_details->>'occurredOn')::date ELSE (r.occurred_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date END)
 ...
 INSERT INTO portal_read_model.medical_document_import(document_id,record_id,source_revision,request,before_record,after_record,source_pages)
 SELECT d.id,r.id,p_revision,request_value,before_value,to_jsonb(saved),COALESCE((
  SELECT jsonb_agg(jsonb_strip_nulls(jsonb_build_object('page',page->'page','pdfText',page->'pdfText',
   'ocrText',page->'ocrText','ocrEngine',page->'ocrEngine')) ORDER BY ord)
  FROM portal_read_model.medical_document_scan scan,
   LATERAL jsonb_array_elements(scan.confirmed_analysis->'pages') WITH ORDINALITY AS source(page,ord)
  WHERE scan.document_id=d.id),'[]'::jsonb)
 FROM portal_read_model.pregnancy_medical_record saved WHERE saved.id=r.id;
 RETURN jsonb_build_object('recordId',r.id,'imported',true);
END $$;

CREATE OR REPLACE FUNCTION public.embe_imported_document_data(p_document_id uuid) RETURNS jsonb
LANGUAGE sql STABLE SECURITY INVOKER SET search_path='' AS $$
 SELECT jsonb_build_object('documentId',i.document_id,'recordId',i.record_id,'importedAt',i.imported_at,
  'sourceSnapshot',jsonb_array_length(i.source_pages)>0,
  'analysis',jsonb_build_object('version',1,'pages',(
   SELECT jsonb_agg(frozen || COALESCE((
    SELECT jsonb_strip_nulls(jsonb_build_object('pdfText',source->'pdfText','ocrText',source->'ocrText','ocrEngine',source->'ocrEngine'))
    FROM jsonb_array_elements(CASE WHEN jsonb_array_length(i.source_pages)>0 THEN i.source_pages
      ELSE COALESCE(s.confirmed_analysis->'pages',s.analysis->'pages','[]'::jsonb) END) AS source
    WHERE source->'page'=frozen->'page' LIMIT 1
   ),'{}'::jsonb) ORDER BY ord)
   FROM jsonb_array_elements(i.request->'analysis'->'pages') WITH ORDINALITY AS fields(frozen,ord))))
 ...
 LEFT JOIN portal_read_model.medical_document_scan s ON s.document_id=i.document_id
