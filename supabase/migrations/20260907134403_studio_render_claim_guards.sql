-- Cancel stale-version renders without overwriting new scripts; reject incomplete output.
CREATE POLICY studio_service_only ON embe_studio.project TO service_role USING (true) WITH CHECK (true);
CREATE POLICY studio_service_only ON embe_studio.notebook TO service_role USING (true) WITH CHECK (true);
CREATE POLICY studio_service_only ON embe_studio.render TO service_role USING (true) WITH CHECK (true);
CREATE POLICY studio_service_only ON embe_studio.worker_state TO service_role USING (true) WITH CHECK (true);

CREATE OR REPLACE FUNCTION public.embe_studio_workspace(p_action text, p_id uuid DEFAULT NULL, p_revision integer DEFAULT NULL, p_payload jsonb DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.embe_studio_worker(p_action text,p_id uuid DEFAULT NULL,p_claim uuid DEFAULT NULL,p_output jsonb DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY INVOKER SET search_path='' AS $$
...
BEGIN
  ...
  ELSIF p_action='finish' THEN
    IF p_output IS NULL OR jsonb_typeof(p_output) <> 'object' OR octet_length(p_output::text)>16000
      OR NOT (p_output ?& ARRAY['video','poster','beats','duration','voiceCredit'])
      OR NOT ((p_output->'video') ?& ARRAY['path','size','mime','checksum'])
      OR NOT ((p_output->'poster') ?& ARRAY['path','size','mime','checksum'])
