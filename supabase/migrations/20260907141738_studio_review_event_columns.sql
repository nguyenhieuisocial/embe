CREATE OR REPLACE FUNCTION public.embe_studio_review(p_action text,p_id uuid DEFAULT NULL,p_revision integer DEFAULT NULL,p_payload jsonb DEFAULT NULL)
...
BEGIN
  IF p_action='list' THEN
    RETURN jsonb_build_object('requests',coalesce((SELECT jsonb_agg(x ORDER BY updated_at DESC) FROM (
      SELECT rr.id,rr.project_id,rr.project_revision,rr.render_id,rr.target,rr.status,rr.revision,rr.created_at,rr.updated_at,
        rr.snapshot->>'title' AS title,(pp.deleted OR pp.revision<>rr.project_revision OR pp.payload<>rr.snapshot) AS stale
      FROM embe_studio.review_request rr JOIN embe_studio.project pp ON pp.id=rr.project_id
      ORDER BY rr.updated_at DESC LIMIT 100
    ) x),'[]'::jsonb));
  ELSIF p_action='get' THEN
    ...
    RETURN jsonb_build_object('request',to_jsonb(q)||jsonb_build_object('stale',p.deleted OR p.revision<>q.project_revision OR p.payload<>q.snapshot),
      'events',coalesce((SELECT jsonb_agg(x ORDER BY created_at,id) FROM (SELECT ev.id,ev.action,ev.note,ev.created_at FROM embe_studio.review_event ev WHERE ev.request_id=q.id ORDER BY ev.id LIMIT 100) x),'[]'::jsonb));
  END IF;
