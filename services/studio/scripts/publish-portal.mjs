...
const { values } = parseArgs({ options: {
  campaign: { type: 'string' }, catalog: { type: 'string' }, 'env-file': { type: 'string' }, output: { type: 'string' }
} });
...
async function main() {
  ...
  for (const rendered of report.videos) {
    ...
    const project = JSON.parse(await sourceFile(resolve(campaign, `${topic.slug}.project.json`)));
    ...
    topics.push({ slug: topic.slug, title: topic.title, pillar: topic.pillar, stage: topic.stage, hookB: topic.hook_b,
      caption: topic.caption, hashtags: topic.hashtags, duration: at, beats,
  ...
  await writeFile(output, JSON.stringify({ version: 1, brand: catalog.brand, checkedAt: catalog.sources_checked_at,
    reviewDue: catalog.review_due, clinicalReview: 'not_reviewed', audio: false, topics, ideas: catalog.research_backlog }, null, 2) + '\n');
  console.log(`Ready for portal: ${topics.length} topics, ${catalog.research_backlog.length} ideas; bucket private`);
}
