...
3. Đặt ghi chú ở chế độ `PUBLIC` và thêm thẻ `#portal`.
...
- Chỉ ghi chú vừa ở chế độ `PUBLIC` vừa có `#portal` mới xuất hiện trên trang gia đình.
- Ghi chú có một trong các thẻ `#private`, `#restricted`, `#medical`, `#health`, `#location` hoặc `#gps` luôn bị chặn, kể cả khi có `#portal`.
- Vị trí, dữ liệu y tế và tệp gốc không được đưa vào kho dữ liệu của Portal.
...
1. Kiểm tra ghi chú đang là `PUBLIC` và có đúng thẻ `#portal`.
