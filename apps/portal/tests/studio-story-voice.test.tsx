...
import {studioDocument,templateDocument,documentScript} from '../src/lib/studio-project';
...
it('saves pronunciation separately, exports it for review, and clears empty overrides',()=>{
  const doc=studioDocument({...templateDocument(),scenes:[{heading:'Ví dụ',text:'DHA 0,5 mg.',speechText:'Đê hát a. 0,5 mg.'}]});
  expect(doc.scenes[0].text).toBe('DHA 0,5 mg.');
  expect(doc.scenes[0].speechText).toBe('Đê hát a. 0,5 mg.');
  expect(documentScript(doc)).toContain('Lời đọc riêng: Đê hát a. 0,5 mg.');
  expect(studioDocument({...doc,scenes:[{...doc.scenes[0],speechText:' '}]}).scenes[0].speechText).toBeUndefined();
...
it('serves only fixed authenticated samples and keeps the old default URL',async()=>{
  auth.denied=true;expect((await GET(new Request('https://embe.hieu.asia/api/studio/voice-preview?voice=thuc-doan-south-v2'))).status).toBe(401);expect(media.serve).not.toHaveBeenCalled();
  auth.denied=false;
