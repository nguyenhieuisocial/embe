...
export async function POST(request: Request, context: Context): Promise<Response> {
  ...
  try {
    const query = await store.rpc("embe_get_baby_medical_document", { p_id: id });
    ...
    const completed = await store.rpc("embe_complete_baby_medical_document", { p_id: id });
...
export async function GET(request: Request, context: Context): Promise<Response> {
  ...
  try {
    ...
    return new Response(downloaded.data, { status: 200, headers: {
      ...
      "content-disposition": `inline; filename*=UTF-8''${encodeURIComponent(String(item.original_filename ?? "tai-lieu"))}`,
