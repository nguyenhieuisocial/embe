...
export default function StageToday() {
  ...
  return <section className="section stage-today"><div className="section-head"><p className="panel-kicker">{early ? "Những tuần đầu sau sinh" : "Mẹ và Bé hôm nay"}</p><h2>{early ? "Nhẹ nhàng chăm cả hai" : "Theo đúng nhịp của gia đình"}</h2></div><div className="stage-today-actions"><a href="/me"><strong>Mẹ hồi phục</strong><span>Ghi điều quan trọng hôm nay</span></a><a href="/be"><strong>Chăm Bé</strong><span>Bú, ngủ, tã và nhiệt độ</span></a><a href="/be/ho-so"><strong>Lịch gần nhất</strong><span>Khám, tiêm và tài liệu</span></a></div></section>;
}
