  ...
  return (<>
    <section className="iphone-health-hub care-tab-panel" hidden={activePanel === "medication"} id="suc-khoe-iphone" aria-labelledby="iphone-health-title">
      ...
      {syncSecret ? <div className="sync-secret" role="status">
        ...
        <p>{connectionState==='expired'?'Mã kết nối không còn hiệu lực. Tạo mã mới để tiếp tục.':connectionState==='offline'?'Đang mất mạng. EmBe tự kiểm tra lại khi có kết nối.':connectionState==='network-error'?'Chưa liên lạc được với máy chủ. EmBe sẽ thử lại khi trang đang mở.':connectionState==='paused'?'Chưa nhận dữ liệu. Thử Manual Export trong Health Auto Export; EmBe tự kiểm tra khi bạn quay lại.':'Đang chờ dữ liệu. Tự kiểm tra trong 2 phút; tiếp tục khi bạn quay lại EmBe.'}</p>
