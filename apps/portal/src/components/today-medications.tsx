...
export default function TodayMedications({compact=false}:{compact?:boolean}={}) {
  ...
  return <section className={`section today-medications${compact?' is-compact':''}`} aria-labelledby="today-medicines-title">
    ...
    {unconfirmed>0?<p className="today-medications-notice">{compact?'Tích khi đã dùng · Cách dùng cần theo bác sĩ.':'Chỉ tích khi đã dùng thực tế. Ghi nhận không thay cho xác nhận cách dùng với bác sĩ.'}</p>:null}
    ...
    <ol className="today-medications-list">{rows.map(({plan,slot,time,status})=><li className="today-medication" data-state={status} key={`${plan.id}-${slot}`}>
      ...
      {status==='taken'?<span className="today-medication-check" title="Đã dùng" aria-label={`${plan.name} lần ${slot}: đã dùng`}>{compact?'✓':'✓ Đã dùng'}</span>
        :<button className="today-medication-check" type="button" disabled={Boolean(saving)||error}
          title="Tích khi đã dùng" aria-label={`Đánh dấu đã dùng ${plan.name} lần ${slot}`} onClick={()=>void markTaken(plan,slot)}>{saving===`${plan.id}-${slot}`?(compact?'…':'Đang lưu…'):compact?<span aria-hidden="true">○</span>:<><span aria-hidden="true">○</span> Tích đã dùng</>}</button>}
      </div>
