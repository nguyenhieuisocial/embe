...
function SourceEditor({ item, onSave, onDelete }: { item: DiscoveryItem; onSave: (item: DiscoveryItem) => boolean; onDelete: () => void }) {
  const [message, setMessage] = useState('');
  return <details className="studio-disclosure"><summary>Sửa ý tưởng & ghi số liệu</summary>
    <form onSubmit={e => { e.preventDefault(); const data = new FormData(e.currentTarget); if (onSave({ ...item, title: String(data.get('title')), note: String(data.get('note')), status: data.get('status') as DiscoveryItem['status'] })) setMessage('Đã lưu thay đổi.'); }}>
    ...
    <form className="discovery-snapshot" onSubmit={e => {
      ...
      try {
        ...
        validateBoard([next]); if (onSave(next)) setMessage('Đã lưu lần ghi nhận.');
...
export default function StudioDiscovery() {
  ...
  useEffect(() => {
    const read = () => { try { stored.current = localStorage.getItem(discoveryKey); setBoard(stored.current ? validateBoard(JSON.parse(stored.current)) : []); setLoaded(true); }
      catch { setLoaded(false); setMessage('Không đọc được sổ trên máy. Không ghi đè dữ liệu cũ; giữ lại dữ liệu để kiểm tra.'); } };
    read(); const listener = (e: StorageEvent) => { if (e.key === discoveryKey || e.key === null) read(); };
  ...
  function persist(next: DiscoveryItem[], text = 'Đã lưu trên thiết bị này.') {
    if (!loaded) return false;
    try { if (localStorage.getItem(discoveryKey) !== stored.current) throw new Error('Sổ đã đổi ở tab khác. Tải lại trang trước khi lưu.');
      const valid = validateBoard(next); const serialized = JSON.stringify(valid);
      try { localStorage.setItem(discoveryKey, serialized); } catch { throw new Error('Chưa lưu được trên máy. Có thể bộ nhớ đã đầy hoặc bị chặn. Nội dung vừa nhập vẫn giữ nguyên.'); }
      stored.current = serialized; setBoard(valid); setMessage(text); return true;
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Chưa lưu được. Hãy giữ lại dữ liệu trước khi đóng trang.'); return false; }
  }
  function add(title: string, url: string, niche: NicheId, note = '') {
  ...
  return <>
    ...
    <details className="studio-disclosure"><summary>Những mẫu Rednote đã xem</summary><p>Học bố cục; không phải các bài đang tăng trưởng. Đã bỏ token khỏi link; nền tảng có thể yêu cầu mở lại bài từ kênh.</p><ul>{seedReferences.map(seed => <li className="discovery-source" key={seed.url}><a href={seed.url} target="_blank" rel="noopener noreferrer">{seed.title}</a><button className="discovery-button" disabled={!loaded} onClick={() => add(seed.title, seed.url, seed.niche)}>Lưu mẫu này</button></li>)}</ul></details>
    <section aria-label="Sổ ý tưởng"><h2 className="studio-research-title">Sổ ý tưởng <small>({board.length})</small></h2>
      <p className="discovery-help">Chỉ lưu trên trình duyệt này, chưa tự đồng bộ hai điện thoại. Xuất sổ để giữ bản sao hoặc nhập trên máy khác. Chỉ ghi ý tưởng công khai, không nhập thông tin sức khỏe riêng.</p>
      <p className="discovery-status" role="status" aria-live="polite">{message}</p>
      {removed && <button className="discovery-button" onClick={() => { try { if (persist(mergeBoard(board, [removed]), 'Đã khôi phục ý tưởng.')) setRemoved(null); } catch { setMessage('Chưa khôi phục được: sổ đã đủ 200 ý tưởng.'); } }}>Hoàn tác xóa</button>}
      <details className="studio-disclosure"><summary>Thêm bài viết, video hoặc kênh</summary><form onSubmit={e => { e.preventDefault(); const form = e.currentTarget; const data = new FormData(form); if (add(String(data.get('title')), String(data.get('url')), nicheId, String(data.get('note')))) form.reset(); }}>
        <label className="studio-search">Tên ý tưởng<input name="title" required maxLength={180} /></label><label className="studio-search">Liên kết gốc<input name="url" type="url" required maxLength={2048} placeholder="https://…" /></label><label className="studio-search">Điều muốn học hoặc làm khác<textarea name="note" rows={2} maxLength={1500} /></label><button className="discovery-button" disabled={!loaded} type="submit">Lưu ý tưởng</button>
      </form></details>
      <details className="studio-disclosure"><summary>Sao lưu & chuyển máy</summary><p>File chứa toàn bộ sổ ý tưởng. Nhập sẽ thêm link mới, giữ nguyên link đã có; không ghi đè ghi chú.</p><button className="discovery-button" disabled={!loaded || !board.length} onClick={() => saveFile('embe-y-tuong.json', JSON.stringify({ version: 1, items: board }), 'application/json')}>Xuất sổ JSON</button>
        <label className="studio-search">Nhập sổ JSON<input type="file" accept="application/json,.json" disabled={!loaded} onChange={async e => { const input = e.currentTarget; const file = input.files?.[0]; if (!file) return; try { if (file.size > 1000000) throw new Error('size'); const value = JSON.parse(await file.text()); if (value.version !== 1) throw new Error('version'); persist(mergeBoard(validateBoard(JSON.parse(stored.current || '[]')), validateBoard(value.items)), 'Đã nhập link mới, giữ ghi chú hiện có.'); } catch { setMessage('Chưa nhập: file không đúng định dạng, quá 1 MB hoặc vượt 200 ý tưởng.'); } finally { input.value = ''; } }} /></label>
      </details>
      ...
      <ul className="studio-ideas">{board.map(item => <li key={item.id}><a className="studio-back" href={item.url} target="_blank" rel="noopener noreferrer">{item.title}</a><p>{niches.find(n => n.id === item.niche)?.label} · {item.status === 'draft' ? 'Đã chọn để viết' : 'Đang tham khảo'}</p><p>{velocity(item).label}</p>{item.note && <p>{item.note}</p>}<button className="discovery-button" onClick={() => saveFile('embe-dan-y.txt', editorialBrief(item), 'text/plain;charset=utf-8')}>Tải dàn ý EmBe</button><SourceEditor key={`${item.id}-${item.title}-${item.status}-${item.note}`} item={item} onSave={next => persist(board.map(old => old.id === item.id ? next : old))} onDelete={() => { if (persist(board.filter(old => old.id !== item.id), 'Đã xóa. Có thể hoàn tác trong lượt mở này.')) setRemoved(item); }} /></li>)}</ul>
