...
export type StudioVoice = { id: 'auto-south' | 'thuc-doan-south-v2' | 'my-duyen-south-v2' | 'kim-thanh-south-v2' | 'thuc-doan-south-v1' | 'my-duyen-south-v1' | 'ai-han-south' | 'piper'; speed: 0.95 | 1 | 1.05 };
...
export function studioDocument(raw: unknown): StudioDocument {
  ...
  if (Object.keys(value).some(k => !['title','stage','caption','scenes','sources','voice','autoRender'].includes(k))) throw new Error('invalid_request');
  if (value.autoRender !== undefined && typeof value.autoRender !== 'boolean') throw new Error('invalid_request');
  let voice: StudioVoice | undefined;
  if (value.voice !== undefined) {
    const v=record(value.voice);
    if (Object.keys(v).some(k=>!['id','speed'].includes(k)) || !['auto-south','thuc-doan-south-v2','my-duyen-south-v2','kim-thanh-south-v2','thuc-doan-south-v1','my-duyen-south-v1','ai-han-south','piper'].includes(String(v.id)) || typeof v.speed!=='number' || ![.95,1,1.05].includes(v.speed)) throw new Error('invalid_request');
  ...
  return { title: text(value.title, 120, true), stage: text(value.stage, 80), caption: text(value.caption, 1500), scenes, sources, ...(voice?{voice}:{}), ...(value.autoRender===undefined?{}:{autoRender:value.autoRender as boolean}) };
}
export function templateDocument(topic?: StudioTopic): StudioDocument {
  const voice: StudioVoice = {id:'auto-south',speed:1};
  return topic ? { title: topic.title, stage: topic.stage, caption: `${topic.caption}\n${topic.hashtags.map(t=>`#${t}`).join(' ')}`, scenes: topic.beats.slice(0,6).map(b=>({heading:b.heading,text:b.text})), sources: topic.sources.slice(0,6).map(s=>({ title: s.publisher, url:s.url })), voice, autoRender:true }
    : { title: 'Ý tưởng mới', stage: 'Mẹ bầu', caption: '', scenes: [{heading:'Mở đầu',text:''},{heading:'Một điều cần biết',text:''},{heading:'Việc nhỏ hôm nay',text:''}], sources: [], voice, autoRender:true };
}
