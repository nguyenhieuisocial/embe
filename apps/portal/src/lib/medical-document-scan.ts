...
export type ExtractedField = { label: string; value: string; unit: string; reference: string; evidence: string; unclear: boolean };
export type ExtractedMedicine = { name: string; ingredients: string; dose: string; frequency: string; instructions: string; evidence: string; unclear: boolean };
export type ExtractedCharge = { label: string; amount: string; currency: string; evidence: string; unclear: boolean };
...
function rows(value: unknown, limits: Record<string, number>, max: number): boolean {
  return Array.isArray(value) && value.length <= max && value.every(row => object(row)
    && exact(row, [...Object.keys(limits), 'unclear']) && typeof row.unclear === 'boolean'
...
export function validDocumentAnalysis(value: unknown): value is DocumentAnalysis {
  ...
  return value.pages.every((page, index) => object(page) && exact(page, ['page', 'kind', 'title', 'fields', 'medicines', 'charges', 'warnings'])
    ...
    && rows(page.fields, { label: 120, value: 1600, unit: 40, reference: 160, evidence: 500 }, 32)
    && rows(page.medicines, { name: 100, ingredients: 1200, dose: 80, frequency: 80, instructions: 200, evidence: 500 }, 12)
    && rows(page.charges, { label: 160, amount: 80, currency: 20, evidence: 500 }, 40)
