...
type ChallengePayload = { id: string; challenge: string; expiresAt: number; purpose: PasskeyPurpose };
export type PasskeyChallenge = { id: string; challenge: string };
...
export function readPasskeyChallenge(
  ...
  try {
    ...
    return { id: payload.id, challenge: payload.challenge };
  } catch {
    return null;
  }
}

export function passkeyChallengeHash(challenge: string, purpose: PasskeyPurpose): string {
  return createHash("sha256").update(`${purpose}\0${challenge}`).digest("hex");
}

export function validChallengeId(value: unknown): value is string {
